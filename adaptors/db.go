...
func (s Store) LogBadRequest(request models.BadRequest) error {
	stmt, err := s.db.Prepare("INSERT INTO badrequest (profile_id, recognized_profiles, current_face, error_type, recognized_time) VALUES ($1, $2, $3, $4, $5)")
	if err != nil {
		return err
	}
	defer stmt.Close()
	_, err = stmt.Exec(request.UserId, pg.Array(request.RecognizedUsers), request.CurrentFace, request.ErrorType, request.RecognizeTime)
	return err
